"""
HATEOAS Agent — Orders example using the Resource (handler-based) API.

Same order management system as orders_declarative.py, but using
the class-based decorator style.
"""

from hateoas_agent import Resource, Runner, gateway, action, state
...
# ============================================================
# Resource definition with decorators
# ============================================================
...
class OrderResource(Resource):
    ...
    def query(self, order_id=None, customer_name=None, status=None):
        matches = []
    ...
    @action(name="approve_order", description="Approve this order for fulfillment",
    ...
    @action(name="cancel_order", description="Cancel this order",
            params={"order_id": "string", "reason": "string"})
    @state("pending", "approved")
    def cancel(self, order_id, reason="No reason given"):
        order = ORDERS[order_id]
        order["status"] = "cancelled"
        order["cancel_reason"] = reason
        return {"success": True, "message": f"Order {order_id} cancelled", "order": order.copy(), "_state": "cancelled"}

    @action(name="modify_order", description="Modify items or details on this order",
            params={"order_id": "string", "changes": "string"})
    @state("pending")
    def modify(self, order_id, changes=""):
        order = ORDERS[order_id]
        order["notes"].append(f"Modified: {changes}")
        return {"success": True, "message": f"Order {order_id} modified", "order": order.copy(), "_state": order["status"]}

    @action(name="add_note", description="Add an internal note to this order",
    ...
    @action(name="ship_order", description="Mark order as shipped and add tracking",
            params={"order_id": "string", "tracking_number": "string", "carrier": "string"})
    @state("approved")
    def ship(self, order_id, tracking_number="", carrier=""):
        order = ORDERS[order_id]
    ...
    @action(name="track_shipment", description="Get tracking details for this shipment",
            params={"order_id": "string"})
    @state("shipped")
    def track(self, order_id):
        ...
        return {
            ...
            "order": order.copy(), "_state": order["status"],
        }

    @action(name="initiate_return", description="Start a return process for this order",
            params={"order_id": "string", "reason": "string", "items": "string (comma-separated)"})
    @state("shipped")
    def initiate_return(self, order_id, reason="", items=""):
        order = ORDERS[order_id]
        order["status"] = "return_initiated"
        order["return_reason"] = reason
        return {
            "success": True, "message": f"Return initiated for order {order_id}",
            "return_label_url": f"https://returns.example.com/label/RET-{order_id}-001",
            "order": order.copy(), "_state": "return_initiated",
        }

    @action(name="reopen_order", description="Reopen this cancelled order",
            params={"order_id": "string"})
    @state("cancelled")
    def reopen(self, order_id):
        order = ORDERS[order_id]
